...
# Function to apply labels with validation
apply_labels() {
    local file="$1"
    local service="$2"
    local labels_file="$TEMP_DIR/labels.yml"
    local temp_file="$TEMP_DIR/temp.yml"
    
    # Create new YAML with labels
    yq eval ".services.$service.labels = load(\"$labels_file\")" "$file" > "$temp_file"
    
    # Validate YAML syntax
    if ! yq eval "." "$temp_file" &>/dev/null; then
        handle_error "YAML validation failed! Changes were not applied."
    fi
    
    # Validate docker-compose configuration
    if ! docker-compose -f "$temp_file" config &>/dev/null; then
        handle_error "Docker Compose validation failed! Changes were not applied."
    fi
...
main() {
    ...
    for tag in $selected_services; do
        ...
        generate_traefik_labels "$service" "$port" > "$TEMP_DIR/labels.yml"
